...
TRACKED_CONTROL_IDS = [
    ...
    "apply_expr",
    "apply_fe",
    "apply_datetime_extract",
    "apply_drop_cols",
    "generate_plot",
    "download_processed_data",
]

def ga4_head_content():
    tracked_ids_js = repr(TRACKED_CONTROL_IDS)
    measurement_id = GA4_MEASUREMENT_ID.replace("'", "\\'")
    variant = APP_VARIANT.replace("'", "\\'")
    ga_script = f"""
...
(function() {{
  ...
  const trackedIds = {tracked_ids_js};
  const analyticsEnabled = !!measurementId && measurementId !== 'G-XXXXXXXXXX';

  const pageTitle = 'Data Studio - ' + variant;

  window.__appVariant = variant;
  window.__ga4Enabled = analyticsEnabled;

  function sendEvent(eventName, params) {{
    if (!window.__ga4Enabled) return;
  ...
  function getCurrentSection() {{
    const activeTab =
      ...
      document.querySelector('.navbar .active');
  ...
  let sectionEnterTs = Date.now();

  function enterSection(sectionName) {{
    currentSection = sectionName || 'unknown';
    sectionEnterTs = Date.now();

    sendEvent('section_view', {{
      section_name: currentSection
    }});

    sendEvent('section_view_' + variant, {{
      section_name: currentSection
    }});
  }}
  ...
  function leaveSection() {{
    ...
    const secondsSpent = Math.max(0, Math.round((Date.now() - sectionEnterTs) / 1000));

    sendEvent('section_time_spent', {{
      section_name: currentSection,
      seconds_spent: secondsSpent
    }});

    sendEvent('section_time_spent_' + variant, {{
      section_name: currentSection,
      seconds_spent: secondsSpent
    }});
  }}
  ...
  if (analyticsEnabled) {{
    ...
    sendEvent('app_loaded', {{}});
    sendEvent('app_loaded_' + variant, {{}});

    setTimeout(function() {{
      enterSection(getCurrentSection());
    }}, 300);
  }} else {{
    console.info('GA4 disabled: set GA4_MEASUREMENT_ID to enable analytics.');
  }}

  // -----------------------------
  // CTR / button click tracking
  // -----------------------------
  document.addEventListener('click', function(e) {{
    const target = e.target && e.target.closest ? e.target.closest('[id]') : null;
    const id = target ? target.id : '';
    if (!window.__ga4Enabled || !id || !trackedIds.includes(id)) return;

    sendEvent('ui_click', {{
      control_id: id,
      section_name: getCurrentSection()
    }});

    sendEvent('ui_click_' + variant, {{
      control_id: id,
      section_name: getCurrentSection()
    }});
  }}, true);
  ...
  // Watches feedback boxes that render success/error alerts
  // -----------------------------
  const feedbackTargets = {{
    upload_status_ui: 'data_load',
    cleaning_feedback_ui: 'cleaning',
    feature_feedback_ui: 'feature_engineering',
    eda_feedback_ui: 'eda'
  }};

  const lastSeenState = {{}};

  function parseTaskStatus(el) {{
    if (!el) return null;

    const successEl = el.querySelector('.alert-success');
    if (successEl) {{
      return {{
        status: 'success',
        message: (successEl.innerText || '').trim()
      }};
    }}

    const errorEl = el.querySelector('.alert-danger');
    if (errorEl) {{
      return {{
        status: 'error',
        message: (errorEl.innerText || '').trim()
      }};
    }}
  ...
  Object.entries(feedbackTargets).forEach(function(entry) {{
    const outputId = entry[0];
    const taskName = entry[1];

    const root = document.getElementById(outputId);
    if (!root) return;

    const observer = new MutationObserver(function() {{
      const state = parseTaskStatus(root);
      if (!state) return;

      const key = taskName + '|' + state.status + '|' + state.message;
      if (lastSeenState[outputId] === key) return;
      lastSeenState[outputId] = key;
      ...
      if (state.status === 'success') {{
        ...
        sendEvent('task_complete_' + variant, {{
          task_name: taskName,
          section_name: getCurrentSection(),
          message_text: state.message
        }});
    ...
    return ui.head_content(
        ...
        ui.tags.script(ui.HTML(ga_script)),
    )
